...
function declaredDecimals(extra) {
  if (!extra || typeof extra !== "object") return null;
  const d = extra.decimals;
  if (typeof d === "number" && Number.isInteger(d) && d >= 0 && d <= 36) return d;
  if (typeof d === "string" && /^\d{1,2}$/.test(d.trim())) {
    const n = Number(d.trim());
    if (n <= 36) return n;
  }
  return null;
}
function describeAsset(network, asset, extra) {
  if (!asset) return { raw: asset, known: false, symbol: null, decimals: null, name: null, decimalsSource: null };
  ...
  if (found) {
    return {
      ...
      decimalsSource: found.decimals != null ? "table" : null
    };
  }
  const declared = declaredDecimals(extra);
  return {
    raw: asset,
    known: false,
    symbol: null,
    decimals: declared,
    // Usually a display name ("World Liberty Financial USD"), not a ticker.
    name: typeof extra?.name === "string" && extra.name.trim() ? extra.name.trim() : null,
    decimalsSource: declared != null ? "challenge" : null
  };
}
...
function formatAmount(amount, network, asset, extra) {
  const assetInfo = describeAsset(network, asset, extra);
  ...
  if (assetInfo.decimals == null) {
    return {
      ...
      symbol: null,
      asset: assetInfo,
      note: `Shown unconverted. This tool does not know the token ${asset || "(none given)"} on ${network || "(no network given)"}, and the challenge did not declare its decimals, so it cannot know how many decimal places to shift. The real price is this number divided by 10^decimals for that token.`
    };
  }
  const value = shiftDecimal(raw, assetInfo.decimals);
  if (!assetInfo.known) {
    const label = assetInfo.name;
    return {
      raw,
      value,
      symbol: null,
      display: label ? `${value} ${label}` : value,
      asset: assetInfo,
      note: `Converted using the ${assetInfo.decimals} decimals the endpoint declared in \`extra.decimals\` for token ${asset} on ${network || "(no network given)"} \u2014 this tool does not have that token in its own table, so the figure is the endpoint's claim about itself rather than one checked against the token contract.`
    };
  }
  return {
    raw,
...
function normalizeAccept(entry, version, topResource) {
  ...
  const amount = formatAmount(amountRaw, entry.network, entry.asset, entry.extra);
  return {
